<x-layout>
    <div class="w-full h-dvh flex flex-col gap-5 md:gap-10 md:p-10 overflow-hidden items-center">
        ...
        <h1 class="font-bold text-[clamp(2rem,5vw,3.5rem)] flex items-start w-full px-5"> Account</h1>
        ...
        <form class="grid grid-cols-1 sm:grid-cols-2 gap-10 w-full lg:w-[55%] px-10 pb-10 md:px-0 overflow-x-auto"
            method="POST" action="{{ route('accounts.store') }}">
            @csrf
            @method("POST")
            ...
            <div class="flex flex-col gap-1">
                ...
                <input id="Email" name="email" type="email"
                    class="outline-none px-3 py-2 border border-[#707070] rounded-lg" value="{{ old('email') }}">
                @error('email')
                <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="flex flex-row gap-5 mt-7">
                <a href="{{ route('accounts.create') }} "
                    class="border border-[#707070] h-10 w-full bg-white text-center rounded-lg hover:bg-[#FD7E14] hover:text-white hover:border-none flex items-center justify-center">
                    No
                </a>
                <button
